...
const TreeFilter = ({menuInfoData}:TreeFilterProps) => {
  ...
  const handleToggle = (event:any, nodeIds:string[]) => {
    if(event.target.closest(".MuiTreeItem-iconContainer")){ //icon 영역을 click할 때만 펼쳐지게 
      let expandedTemp = expanded;
    ...
    <div>
      <Box sx={{padding: 1, display: 'flex', flexDirection:'row'}} >
        <Box sx={{ display: 'flex', paddingRight: 1 }}>
          ...
          <TextField id="input-with-sx" variant="standard" onKeyUp={onFilterMouseUp} />
        </Box>
        <Box sx={{display:'flex'}}>
          <Button variant="contained" style={{backgroundColor: MAINBLUE, minWidth: '50px'}} onClick={() => { navigate('/edit'); window.location.reload(); }}>
            {"편집하기"}
          </Button>
